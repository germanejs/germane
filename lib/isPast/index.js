 ...
 * @name isPast
 */
function isPast(current, relative) {
  ...
  const currentDate = (typeof current === "number" ? new Date(current) : current).getTime();
  const relativeDate = (typeof relative === "number" ? new Date(relative) : relative).getTime();

  return currentDate > relativeDate;
}
