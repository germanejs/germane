...
/**
 * @param {Date|Number} startDate Date which is to be checked as the previous day.
 ...
 */
function isYesterday(startDate, endDate) {
  ...
  const nOne = Object.prototype.toString.call(startDate) === "[object Date]"
    ? new Date(startDate)
    : germane(startDate, "UTC");
  const nTwo = Object.prototype.toString.call(endDate) === "[object Date]"
    ? new Date(endDate)
    : germane(endDate, "UTC");
