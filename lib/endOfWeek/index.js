...
const { isDateOrNumber, isObject, isNumber } = require("../_util/validations/validatePropTypes");
 ...
 * ```
 */
function endOfWeek(date = Date.now(), options = { weekStartsOn: 0 }) {
  ...
  if (!isNumber(options.weekStartsOn)) return new TypeError("Invalid option: weekStartsOn expects a number");
  if (options.weekStartsOn > 6 || options.weekStartsOn < 0) return new RangeError("Invalid opton: weekStartsOn is out of range (0~6)");
