...
const { startOfMonthHandler } = require("../_util/evaluations/startOf");
 ...
 * @summary returns the start of month for current date, shifting it to UTC time.
 *
 ...
 * ```js
 * startOfMonth(new Date("2109-09-11"));
 * //=> "2109-09-01T(shifted hours):(shifted minutes):00.000Z";
 * ```
 */
function startOfMonth(date) {
  if (!isDateOrNumber(date)) return new TypeError("Invalid Date");
  if (!validateDate(date)) return new RangeError("Invalid Date");
  const startOfM = startOfMonthHandler(typeof date === "number" ? new Date(date) : date);
  return startOfM;
}
