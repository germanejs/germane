 ...
 * @fileoverview Returns the end of a given years decade.
 */

"use strict";

const { endOfDecadeHandler } = require("../_util/evaluations/endOf");
 ...
 * @summary returns the end of decade for current date, shifting it to UTC time.
 *
 ...
 * ```js
 * endOfDecade(new Date("2109-09-11 11:09:11Z"));
 * //=> "2110-12-31T(shifted hours):(shifted minutes):59.999Z";
 * ```
 */
function endOfDecade(date = Date.now()) {
  if (!isDateOrNumber(date)) return new TypeError("Invalid Date");
  if (!validateDate(date)) return new RangeError("Invalid Date");
  const endOfD = endOfDecadeHandler(typeof date === "number" ? new Date(date) : date);
  return endOfD;
}
