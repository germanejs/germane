...
function isLastWeek(current, relative) {
  ...
  const eOfPreviousWeek = endOfWeekHandler(x).getTime();
  const sOfPreviousWeek = startOfWeekHandler(x).getTime();
  const range = inRangeOf(relativeDate.getTime(), sOfPreviousWeek, eOfPreviousWeek);
  return range;
}
