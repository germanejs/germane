...
function getTimeZoneOffset(timezone, stamp) {
  ...
  // Added zero to prevent -0 results.
  const stampMod1000 = (stamp % 1000) + 0;
  // stamp modded by 1000 will return the milliseconds.
  const ms = stampMod1000 < 0 ? 1000 + stampMod1000 : stampMod1000 + 0;
  const posix = epoch(year, month, day, hour, minute, second, ms);

  const anon = (posix - stamp) / 1000 / 60;
  ...
  return {
    offsetInMinutes: offsetInMinutes < 0 ? Math.abs(offsetInMinutes) : -offsetInMinutes,
