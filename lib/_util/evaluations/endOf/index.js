...
function endOfQuarterHandler(date) {
  ...
  const endOfPrevYear = epoch(year - 1, 12, 31, 23, 59, 59, 999, date.getTimezoneOffset());
  const quarterEnd = findQuarterEnd(quarter, year);
  const newDate = endOfPrevYear + quarterEnd * 86400000;
...
function endOfMinuteHandler(date) {
  ...
  const newDate = epoch(year, month, day, hour, date.getMinutes(), 59, 999, date.getTimezoneOffset());
  return isGermane(date) ? germane(newDate, date.timezone) : new Date(newDate);
}
...
function endOfYearHandler(date) {
  const year = date.getFullYear();
  const newDate = epoch(year, 12, 31, 23, 59, 59, 999, date.getTimezoneOffset());
  return isGermane(date) ? germane(newDate, date.timezone) : new Date(newDate);
}
