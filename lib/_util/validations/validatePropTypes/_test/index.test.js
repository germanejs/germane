...
describe("Validations for property types or parameters to date functions", () => {
  test("should return true", () => {
    ...
    expect(isDateOrNumber(germane())).toBe(true);
    ...
    expect(isValidDateArray([new Date(), germane(), 13141131e5, germane("2019-11")], validateDate)).toBe(true);

    expect(isLocale({locale: enUS})).toBe(true);
    expect(isLocale({locale: fr})).toBe(true)
  });
  ...
  test("Should return false", () => {
    ...
    expect(isValidDateArray([new Date(), germane(), false, germane("2019 11")], validateDate)).toBe(false);
...
describe("ValidateISO function validates ISO or RFC5822 or 2822 strings and returns a object of boolean values", () => {
  ...
  test("should return 1901109030000", () => {
    expect(validateISO("Sat, 30 Mar 2030 14:50:30 +1300")).toStrictEqual({
      isExpanded: false,
      isBasic: false,
      isASP: false,
      isRFC: true,
