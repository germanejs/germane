...
describe("creates a duration", () => {
  ...
  test("should return errors", () => {
    expect(createDuration("")).toStrictEqual(new RangeError("Invalid duration: empty string"));

    expect(createDuration({})).toStrictEqual(new TypeError("Invalid duration"));
    ...
    expect(createDuration([null, new Date()])).toStrictEqual(
      new RangeError("Invalid duration: duration array contains non Date value(s)"),
    );
  });
});
