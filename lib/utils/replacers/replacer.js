 ...
 * @returns {string} returns a string with the formatted replacements if there is a format, else it returns the string
 * @name replacer
 * @throws {Invalid format string} throws an error when the str parameter contains an invalid format
 * @example replacer("YY %d", {"YY": "Bar", "%d": "foo"}, /^YY|%[a-zA-Z]/, ["YY", "YYYY", "%d", "%a"])
 */
exports.replacer = function(formatString, replacement, regex) {
	const newReplacement = Object.assign({}, replacement);
	let newStr = formatString;
	const newArray = newStr.match(regex);
	if (newArray === null || newArray === undefined || !newArray.length)
		return formatString;
