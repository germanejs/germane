...
const ordinalOfPrevYear = require("./ordinal_of_prev_year");
 ...
 * @description Epoch function uses the specified datetime units and returns a unix timestamp of the given date as an ISO8601 format.
 // eslint-disable-next-line max-len
 * **Note**: if not parameter is passed the function simple returns 0. meaning that the default parameters is 1970 01 01 00:00:00.
 * @example ```javascript
...
function epoch(year = 1970, month = 1, day = 1, hour = 0, minute = 0, second = 0, ms = 0, tz = 0) {
  const unixEpoch = {
    ...
    month: 1,
    day: 1,
    hour: floor(tz / 60) + hour,
    minute: floor(tz % 60) + minute,
    second: second,
    ms: ms,
  };
