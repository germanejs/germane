...
/**
 *
 * @param {any} start {year, month, date, hour, minute, second}
 * @param {Number} months
 * @returns {Number}
...
function addOrSubMonths(
  ...
  const startEpoch = epoch(...Object.values(start));
  ...
  if (type === "sub") {
    ...
    end = epoch(year, month, checkDay, start.hour, start.minute, start.second);
  } else if (type === "add") {
    month = Math.abs(
