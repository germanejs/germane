...
// console.log(endOfMonth(germane("2098-03-01T09:09:27.091Z")).toString());
// console.log(
...
setTimer.loop(({
  totalDay, hour, minute, second, millisecond: ms,
}) => {
  console.log(
    `Loop one -- Days: ${totalDay} | Time: ${hour < 10 ? "0" + hour : hour}:${
      minute < 10 ? "0" + minute : minute
    }:${second < 10 ? "0" + second : second}.${ms % 100}`,
  );
});
if (setTimer.isIdle()) {
  setTimer.loop(({
    ...
    console.log(
      `Loop two -- Days: ${totalDay} | Time: ${hour < 10 ? "0" + hour : hour}:${
        minute < 10 ? "0" + minute : minute
      }:${second < 10 ? "0" + second : second}.${ms % 100}`,
    );
  });
}
console.log(createDuration("P12Y8M9DT45H34M12S"));

// setTimeout(() => setTimer.pause(), 5000);

// setTimeout(() => setTimer.start(), 10000);

// setTimeout(() => setTimer.restart(), 13000);

setTimer.append(10000);

// setTimeout(() => setTimer.stop(), 20000);

// setTimer.loop(() => console.log("Hello World"));

console.log(differenceInMilliseconds(new Date(), new Date("2019-11-27")));
