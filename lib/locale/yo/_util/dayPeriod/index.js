...
function dayPeriod(hour, min, sec, ms) {
  ...
  // 05:00:00 - 11:59:59.999 morning

  const morningDurationStart = (5 * 3600 + 0 * 60 + 0) * 1000;
  ...
  const night1 = inRangeOf(timeToms, night1DurationStart, night1DurationEnd);

  // 00:00:00.001 - 04:59:59.999 night 2.
  const night2DurationStart = (0 * 3600 + 0 * 60 + 0) * 1000 + 1;
  const night2DurationEnd = (4 * 3600 + 59 * 60 + 59) * 1000 + 999;
  const night2 = inRangeOf(timeToms, night2DurationStart, night2DurationEnd);

  const meridiem = hour > 11 ? "pm" : "am";

  // prettier-ignore
  const period = midnight ? "midnight" : night1 ? "night" : night2 ? "night2" : morning ? "morning" : noon ? "noon" : afternoon ? "afternoon" : evening ? "evening" : meridiem;
