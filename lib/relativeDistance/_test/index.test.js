...
describe("relativeDistance computes the distance between two dates", () => {
  test("should return in two months", () => {
    expect(
      relativeDistance(new Date("2019-09-11 23:11"), new Date("2019-11-15"), { addSuffix: true }),
    ).toBe("in 2 months");
  });
  test("should return 45 years ago", () => {
    expect(
      relativeDistance(new Date("2019-11-12 23:11"), new Date("1973-11-15"), { addSuffix: true }),
    ).toBe("45 years ago");
  });
  test("should return 3 weeks ago", () => {
    expect(
      relativeDistance(new Date("2019-11-12 23:11"), new Date("2019-10-21"), { addSuffix: true }),
    ).toBe("3 weeks ago");
  });
  test("should return 2 days", () => {
    expect(
      relativeDistance(new Date("2019-10-23 23:11"), new Date("2019-10-21"), { addSuffix: false }),
    ).toBe("2 days");
  });
