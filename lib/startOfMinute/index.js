...
const { startOfMinuteHandler } = require("../_util/evaluations/startOf");
 ...
 * @summary returns the start of minute for current date, shifting it to UTC time.
 *
 ...
 * ```js
 * startOfMinute(new Date("2109-09-11"));
...
function startOfMinute(date) {
  ...
  const startOfM = startOfMinuteHandler(typeof date === "number" ? new Date(date) : date);
  return startOfM;
}
