...
function isSameWeek(current, relative) {
  ...
  const eOfWeek = endOfWeekHandler(x);
  const sOfWeek = startOfWeekHandler(x);
  const endOf = eOfWeek.getTime() - eOfWeek.getTimezoneOffset() * 60 * 1000;
  const startOf = sOfWeek.getTime() - sOfWeek.getTimezoneOffset() * 60 * 1000;
