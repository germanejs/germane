...
const { startOfDayHandler } = require("../_util/evaluations/startOf");
 ...
 * @summary returns the start of day for current date, shifting it to UTC time.
 *
 ...
 * ```js
 * startOfDay(new Date("2109-09-11")); //=> "2109-09-11T(shifted hours):(shifted minutes):00.000Z";
 * ```
 */
function startOfDay(date) {
  if (!isDateOrNumber(date)) return new TypeError("Invalid Date");
  if (!validateDate(date)) return new RangeError("Invalid Date");
  const startOfD = startOfDayHandler(typeof date === "number" ? new Date(date) : date);
  return startOfD;
}
